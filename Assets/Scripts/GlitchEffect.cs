...
public class GlitchEffect : ImageEffectBase {
	...
	void OnRenderImage (RenderTexture source, RenderTexture destination) {
		...
		if (time > 0){
			time -= Time.deltaTime;
			material.SetFloat("displace", intense * (time / startTime));
			material.SetFloat("scale", 1 - Random.value * intense * (time / startTime));
        }
        else
	...
	private float intense, time, startTime;

	public void Glitch(float intensity, float time)
	{
		intense = intensity;
		this.time = time;
		startTime = time;
